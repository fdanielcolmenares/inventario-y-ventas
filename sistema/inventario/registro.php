<?PHP
	...
	$sql="insert into articulo (codigo_articulo, descripcion, fecha, estado, codigo_usuario)
	 values ('".$_POST["codigo"]."','".$_POST["desc"]."','".date("Y-m-d")."','".$_POST["estado"]."','".$_SESSION["idUsuario"]."')";
	$res=mysql_query($sql,$bd);
	
	$sql2="insert into articulo_imagen (imagen, tamano, codigo_articulo)
	 values ('".$nombre."',1,'".$_POST["codigo"]."')";
	$res2=mysql_query($sql2,$bd);
	
	if($res && $res2){
		echo'
			 <script type="text/javascript">
  				window.location="inventario.php?E=1";
			 </script>
		';	
	}
	else{
		echo'
			 <script type="text/javascript">
  				window.location="inventario.php?E=0";
			 </script>
		';	
	}
